...
static SDL_Scancode TranslateKeycode(int keycode, int hardware_scancode)
{
    SDL_Scancode scancode = SDL_SCANCODE_UNKNOWN;

    /* Android's keycode can reflect the active ChromeOS keyboard layout. FT2
       needs the physical key position for its piano rows, just like the
       desktop SDL backends. Android scan codes originate from Linux evdev on
       physical keyboards, so translate them with SDL's existing Linux table.
       Virtual keyboards and devices without a useful scan code retain the
       standard Android keycode path below. */
    if (hardware_scancode > 0 && hardware_scancode < SDL_arraysize(linux_scancode_table)) {
        scancode = linux_scancode_table[hardware_scancode];
    }
    if (scancode == SDL_SCANCODE_UNKNOWN && keycode >= 0 && keycode < SDL_arraysize(Android_Keycodes)) {
        scancode = Android_Keycodes[keycode];
    }
    if (scancode == SDL_SCANCODE_UNKNOWN) {
        __android_log_print(ANDROID_LOG_INFO, "SDL", "Unknown keycode %d, scancode %d", keycode, hardware_scancode);
    }
    return scancode;
}

int Android_OnKeyDown(int keycode, int scancode)
{
    return SDL_SendKeyboardKey(SDL_PRESSED, TranslateKeycode(keycode, scancode));
}

int Android_OnKeyUp(int keycode, int scancode)
{
    return SDL_SendKeyboardKey(SDL_RELEASED, TranslateKeycode(keycode, scancode));
}
